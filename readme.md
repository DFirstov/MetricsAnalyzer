...
- Перед стартом нагрузки вызывает `POST /api/stocks/seed/{count}` (сейчас 1 000 000), чтобы подготовить большой набор тестовых данных.
- Запускает сценарий `Invest App Load`, который 50 раз в секунду в течение 5 минут выполняет `GET /api/stocks/{ticker}`.
- Для каждого запроса выбирается случайный тикер с предвзятым распределением (чаще используются «ранние» тикеры), чтобы лучше увидеть поведение cache-aside под нагрузкой.

## Что умеет MarketData.Api сейчас

- `POST /api/stocks/seed/{count}` — очищает таблицу `Stocks` и заново заполняет её `count` синтетическими акциями в формате `TKR0000001`, `TKR0000002`, ...
